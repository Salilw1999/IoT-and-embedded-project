...
char auth[] = "MLfecHdqYcTeYG2DgIAxWILMQrmhwhoo";
...
BLYNK_WRITE(V5) // Executes when the value of virtual pin 0 changes
 {
  if(param.asInt() == 1)
    ...
    digitalWrite(RELAY_PIN,HIGH);  // Set digital pin 2 LOW 
    }
 }
...
servo1.write(param.asInt());
 ...
 void loop() {
  ...
  if (moistureLevel < MOISTURE_THRESHOLD_OFF) {
    digitalWrite(RELAY_PIN, HIGH); // Turn on relay
  }
  // If the moisture level is above the threshold, turn off the relay
  if (moistureLevel >= MOISTURE_THRESHOLD_ON) {
